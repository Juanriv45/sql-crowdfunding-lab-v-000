...
"SELECT projects.title, SUM(pledges.amount)-SUM(projects.funding_goal) FROM projects JOIN pledges ON Projects.id = Pledges.project_id GROUP BY Projects.title HAVING (SUM(pledges.amount) - SUM(projects.funding_goal)) >= 0 ;"
end
